...
export default function NotificationsPage() {
  ...
  const getIconForType = (type: string) => {
    switch (type) {
      case 'APPLICATION_REVIEWING': return 'visibility';
      case 'APPLICATION_INTERVIEWING': return 'calendar_month';
      case 'APPLICATION_HIRED': return 'celebration';
      case 'APPLICATION_REJECTED': return 'cancel';
      case 'JOB_CLOSED': return 'work_off';
  ...
  const getColorClassForType = (type: string) => {
    if (type.startsWith('APPLICATION_')) return 'border-purple-500 text-purple-500';
    if (type.startsWith('JOB_')) return 'border-amber-500 text-amber-500';
    if (type.startsWith('ACCOUNT_')) return 'border-red-500 text-red-500';
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm font-medium" style={{ color: "var(--db-text-muted)" }}>
              Stay updated on your applications and account status.
            </p>
          </div>
          {unreadCount > 0 && (
            <button
